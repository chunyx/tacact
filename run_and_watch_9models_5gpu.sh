...
# One-command runner:
# - starts 5-GPU deep-only main experiment
# - auto-detects RUN_ROOT from startup logs
# - auto-starts compact watcher dashboard
#
# Supported env passthrough (consumed by run_main_9models_5gpu.sh):
#   DATA_ROOT, OUTPUT_ROOT, SEED, EPOCHS, SPLIT_MODE, BATCH_SIZE, NUM_WORKERS,
#   AMP_INFER, SKIP_CACHE_WARMUP, NO_PRELOAD
...
TMP_RUN_LOG="$(mktemp -t tacact_run_main_5gpu_XXXX.log)"
...
cleanup() {
  ...
  rm -f "${TMP_RUN_LOG}" >/dev/null 2>&1 || true
}
trap cleanup INT TERM

# Run training in background and capture all output.
bash run_main_9models_5gpu.sh >"${TMP_RUN_LOG}" 2>&1 &
TRAIN_PID=$!
echo "Training PID: ${TRAIN_PID}"
echo "Capturing startup logs at: ${TMP_RUN_LOG}"

# Parse [Run Root] from startup output.
run_root=""
start_ts="$(date +%s)"
while true; do
...
python watch_main_9models_5gpu.py --run_root "${run_root}"

# Watcher exits when jobs are done/failed/missing. Ensure training also ended.
wait "${TRAIN_PID}" || {
  rc=$?
...
echo "Main logs:"
echo "  ${run_root}/logs/gpu0.log"
echo "  ${run_root}/logs/gpu1.log"
echo "  ${run_root}/logs/gpu2.log"
echo "  ${run_root}/logs/gpu3.log"
echo "  ${run_root}/logs/gpu4.log"

...
ROOT = Path.cwd()
OUT_DIR = Path.cwd() / "outputs" / "diagnosis" / "lenet_lstm"
EX_DIR = OUT_DIR / "confused_pair_examples"
TacActDataset = None


def _import_project_modules(project_root: Path) -> None:
    global TacActDataset
    if str(project_root) not in sys.path:
        sys.path.insert(0, str(project_root))
    from data import TacActDataset as _TacActDataset
...
def discover_runs(project_root: Path) -> pd.DataFrame:
    rows: List[Dict[str, Any]] = []
    for root in project_root.glob("outputs*"):
        if not root.is_dir():
            continue
...
def compute_sequence_stats_for_sample(ds, sample_id: int) -> Dict[str, Any]:
...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Diagnose existing LeNet_LSTM experiment outputs without retraining.")
    parser.add_argument("--project_root", type=Path, required=True, help="Project root directory, e.g. /home/yaxin/tacact")
    parser.add_argument("--output_dir", type=str, required=True, help="Output directory for diagnosis artifacts")
    return parser
...
def main() -> None:
    ...
    if not pred_df.empty:
        ...
        if "top1_confidence" in pred_df.columns:
            ...
            plt.tight_layout()
            plt.savefig(OUT_DIR / "wrong_confidence_distribution.png", dpi=220, bbox_inches="tight")
            plt.close(fig)
            ...
            if not corr_conf.empty:
                vals.append(corr_conf.to_numpy())
                labels.append("Correct")
            if not wrong_conf.empty:
                vals.append(wrong_conf.to_numpy())
                labels.append("Wrong")
            if vals:
                ax.boxplot(vals, tick_labels=labels, showmeans=True)
            ...
            ax.grid(True, axis="y", alpha=0.25)
            plt.tight_layout()
            plt.savefig(OUT_DIR / "correct_vs_wrong_confidence.png", dpi=220, bbox_inches="tight")
            plt.close(fig)
    else:
    ...
    if not hist_df.empty:
        ...
        plt.tight_layout()
        plt.savefig(OUT_DIR / "train_val_loss_curve.png", dpi=220, bbox_inches="tight")
        plt.close(fig)
        ...
        ax.legend(frameon=False)
        plt.tight_layout()
        plt.savefig(OUT_DIR / "val_f1_curve.png", dpi=220, bbox_inches="tight")
        plt.close(fig)
    else:
    ...
    if not weakest_df.empty:
        for _, row in weakest_df.iterrows():
            weakest_lines += (
                f"- {row['class_name']} (class_id={row['class_id']}): "
    ...
    if not compare_df.empty:
        ...
        try:
            ...
            pass
    if not subject_df.empty and subject_df["macro_f1"].max() - subject_df["macro_f1"].min() > 0.15:
